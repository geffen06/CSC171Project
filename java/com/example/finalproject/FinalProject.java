...
public class FinalProject extends Application {
    ...
    public void start(Stage primaryStage) {
        ...
        player = new Rectangle(width / 2 - playerWidth / 2, height - playerHeight - 20, playerWidth, playerHeight);
    ...
    private void fireBullet() {
        Rectangle bullet = new Rectangle(player.getX() + playerWidth / 2 - bulletWidth / 2, player.getY(), bulletWidth, bulletHeight);
        bullet.setFill(Color.RED);
        bullets.add(bullet);
    }

    // Update the bullets, moving them forward
    private void updateBullets() {
        Iterator<Rectangle> iterator = bullets.iterator();
        while (iterator.hasNext()) {
            Rectangle bullet = iterator.next();
            bullet.setX(bullet.getX() - 10); // Move the bullet left
